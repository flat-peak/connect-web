...
import {isEqualObjects, throwOnApiError} from '@flat-peak/javascript-sdk';


export const fetchTariff = createAsyncThunk(
  "tariff/fetch",
  async ({ tariffId, apiKey }) => {
    ...
    const request = await fetch(`/api/tariff?${qs.toString()}`);
...
export const fetchProvider = createAsyncThunk(
  ...
  async ({ providerId, apiKey }) => {
    ...
    const request = await fetch(`/api/provider?${qs.toString()}`);
    const result = await request.json();
    return throwOnApiError(result);
  }
);
...
export const tariffSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchTariff.fulfilled, (state, action) => {
      state.plan = action.payload;
    });
